...
import { isCuttingStart, isCuttingEnd, runFfmpegWithProgress, getFfCommandLine, getDuration, createChaptersFromSegments, readFileFfprobeMeta, getExperimentalArgs, getVideoTimescaleArgs, logStdoutStderr, runFfmpegConcat, runFfmpegVoid, runFfmpegText } from '../ffmpeg';
...
function useFfmpegOperations({ filePath, treatInputFileModifiedTimeAsStart, treatOutputFileModifiedTimeAsStart, isEncoding, lossyMode, enableOverwriteOutput, outputPlaybackRate, cutFromAdjustmentFrames, cutToAdjustmentFrames, appendLastCommandsLog, encCustomBitrate, appendFfmpegCommandLog }: {
  ...
  const concatFiles = useCallback(async ({ paths, outDir, outPath, metadataFromPath, includeAllStreams, streams, outFormat, ffmpegExperimental, onProgress = () => undefined, preserveMovData, movFastStart, chapters, preserveMetadataOnMerge, videoTimebase }: {
    ...
    const durations = await pMap(paths, async (path) => await getDuration(path), { concurrency: 1 });
    ...
    try {
      ...
      const ffmpegArgs = [
        ...
        ...await getExperimentalArgs(ffmpegExperimental),

        ...await getVideoTimescaleArgs(videoTimebase),
  ...
  const cutEncodeSmartPart = useCallback(async ({ cutFrom, cutTo, outPath, outFormat, videoCodec, videoBitrate, videoTimebase, allFilesMeta, copyFileStreams, videoStreamIndex, ffmpegExperimental }: {
    ...
    await appendFfmpegCommandLog(ffmpegArgs);
    await runFfmpegVoid(ffmpegArgs);
  ...
  const extractNonAttachmentStreams = useCallback(async ({ customOutDir, streams }: {
    ...
    const stdoutText = await runFfmpegText(ffmpegArgs);
  ...
  const extractAttachmentStreams = useCallback(async ({ customOutDir, streams }: {
    ...
    try {
      await appendFfmpegCommandLog(ffmpegArgs);
      const res = await runFfmpegText(ffmpegArgs);
      console.log(res);
      // const { stdout } = await ffmpeg.runFfmpeg(ffmpegArgs);
      // console.log(new TextDecoder().decode(stdout));
    } catch (err) {
