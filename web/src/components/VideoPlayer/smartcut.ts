...
import { type IFfmpeg, TOKENS, type FFprobeStream, type IUtils } from 'lossless-cut-application';
import { UserFacingError } from './errors';
...
export async function needsSmartCut({ path, desiredCutFrom, videoStream }: {
  ...
  const keyframeAtExactTime = ffmpeg.findKeyframeAtExactTime(keyframes, desiredCutFrom);
  ...
  let nextKeyframe = ffmpeg.findNextKeyframe(keyframes, desiredCutFrom);

  if (nextKeyframe == null) {
    // try again with a larger window
    keyframes = await readKeyframes(60);
    nextKeyframe = ffmpeg.findNextKeyframe(keyframes, desiredCutFrom);
  }
