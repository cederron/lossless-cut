...
import { type IFfmpeg, TOKENS, type CaptureFormat, type IUtils } from 'lossless-cut-application';
...
export default ({ appendFfmpegCommandLog, formatTimecode, treatInputFileModifiedTimeAsStart, treatOutputFileModifiedTimeAsStart, fileDuration }: {
  ...
  const captureFramesRange = useCallback(async ({ customOutDir, filePath, fps, fromTime, toTime, estimatedMaxNumFiles, captureFormat, quality, filter, onProgress, outputTimestamps }: {
    ...
    if (!outputTimestamps) {
      ...
      const outPathTemplate = utils.getSuffixedOutPath({ customOutDir, filePath, nameSuffix: getSuffix(`%0${numDigits}d`) });
      const firstFileOutPath = utils.getSuffixedOutPath({ customOutDir, filePath, nameSuffix: getSuffix(`${'1'.padStart(numDigits, '0')}`) }); // mimic ffmpeg output
    ...
    const outPathTemplate = utils.getSuffixedOutPath({ customOutDir, filePath, nameSuffix: getSuffix(`${tmpSuffix}%d`) });
    ...
    const files = await utils.readdir(outDir);

    const matches = files.flatMap((fileName) => {
      const escapedRegexp = escapeRegExp(utils.getSuffixedFileName(filePath, tmpSuffix));
    ...
    const outPaths = await pMap(matches, async ({ fileName, frameNum }) => {
      ...
      const renameFromPath = utils.getOutPath({ customOutDir, filePath, fileName });
      const renameToPath = utils.getOutPath({ customOutDir, filePath, fileName: utils.getSuffixedFileName(filePath, getSuffix(duration)) });
      await utils.renameWithRetry(renameFromPath, renameToPath);
      return renameToPath;
    }, { concurrency: 1 });
  ...
  const captureFrameFromFfmpeg = useCallback(async ({ customOutDir, filePath, time, captureFormat, quality }: {
    ...
    const outPath = utils.getSuffixedOutPath({ customOutDir, filePath, nameSuffix });
  ...
  const captureFrameFromTag = useCallback(async ({ customOutDir, filePath, time, captureFormat, quality, video }: {
    ...
    const ext = utils.getMimeExtension(dataUri.type);
    const timecode = formatTimecode({ seconds: time, fileNameFriendly: true });

    const outPath = utils.getSuffixedOutPath({ customOutDir, filePath, nameSuffix: `${timecode}.${ext}` });
    await utils.writeFile(outPath, new Uint8Array(dataUri.buffer));
