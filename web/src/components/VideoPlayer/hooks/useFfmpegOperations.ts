...
import { RefuseOverwriteError, TOKENS, type AllFilesMeta, type AvoidNegativeTs, type Chapter, type CopyfileStreams, type FFprobeStream, type Html5ifyMode, type IFfmpeg, type IPlatform, type IUtils, type LiteFFprobeStream, type LossyMode, type PreserveMetadata, type SegmentToExport } from 'lossless-cut-application';
...
import { deleteDispositionValue, type CustomTagsByFile, type ParamsByStreamId } from '../types';
...
function useFfmpegOperations({ filePath, treatInputFileModifiedTimeAsStart, treatOutputFileModifiedTimeAsStart, isEncoding, lossyMode, enableOverwriteOutput, outputPlaybackRate, cutFromAdjustmentFrames, cutToAdjustmentFrames, appendLastCommandsLog, encCustomBitrate, appendFfmpegCommandLog }: {
  ...
  appendFfmpegCommandLog: (args: string[]) => void,
    ...
    const platform = useInjection<IPlatform>(TOKENS.Platform);
  ...
  const concatFiles = useCallback(async ({ paths, outDir, outPath, metadataFromPath, includeAllStreams, streams, outFormat, ffmpegExperimental, onProgress = () => undefined, preserveMovData, movFastStart, chapters, preserveMetadataOnMerge, videoTimebase }: {
    ...
    try {
      ...
      const ffmpegArgs = [
        ...
        ...ffmpeg.getExperimentalArgs(ffmpegExperimental),

        ...ffmpeg.getVideoTimescaleArgs(videoTimebase),
  ...
  const losslessCutSingle = useCallback(async ({
    ...
    const cuttingStart = ffmpeg.isCuttingStart(cutFrom);
    ...
    const cuttingEnd = ffmpeg.isCuttingEnd(cutTo, fileDuration);
  ...
  const cutMultiple = useCallback(async ({
    ...
    async function cutSegment({ start: desiredCutFrom, end: cutTo }: { start: number, end: number }, i: number) {
      ...
      const ext = utils.getOutFileExtension({ isCustomFormatSelected: true, outFormat, filePath });
      ...
      const losslessPartOutPath = segmentNeedsSmartCut
        ? utils.getSuffixedOutPath({ customOutDir, filePath, nameSuffix: `smartcut-segment-copy-${i}${ext}` })
        : finalOutPath;
      ...
      const smartCutEncodedPartOutPath = utils.getSuffixedOutPath({ customOutDir, filePath, nameSuffix: `smartcut-segment-encode-${i}${ext}` });
  ...
  const concatCutSegments = useCallback(async ({ customOutDir, outFormat, segmentPaths, ffmpegExperimental, onProgress, preserveMovData, movFastStart, chapterNames, preserveMetadataOnMerge, mergedOutFilePath }: {
    ...
    const outDir = utils.getOutDir(customOutDir, filePath);
  ...
  const html5ify = useCallback(async ({ customOutDir, filePath: filePathArg, speed, hasAudio, hasVideo, onProgress }: {
    ...
    const outPath = utils.getHtml5ifiedPath(customOutDir, filePathArg, speed);
  ...
  const fixInvalidDuration = useCallback(async ({ fileFormat, customOutDir, onProgress }: {
    ...
    invariant(filePath != null);
    const ext = utils.getOutFileExtension({ outFormat: fileFormat, filePath });
    const outPath = utils.getSuffixedOutPath({ customOutDir, filePath, nameSuffix: `reformatted${ext}` });
  ...
  const extractNonAttachmentStreams = useCallback(async ({ customOutDir, streams }: {
    ...
    const outPaths = await pMap(outStreams, async ({ index, codec, type, format: { format, ext } }) => {
      const outPath = utils.getSuffixedOutPath({ customOutDir, filePath, nameSuffix: `stream-${index}-${type}-${codec}.${ext}` });
