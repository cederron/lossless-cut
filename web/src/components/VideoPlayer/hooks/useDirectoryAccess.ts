...
import { askForInputDir } from '../dialogs';
...
export default function useDirectoryAccess({ setCustomOutDir }: { setCustomOutDir: (a: string | undefined) => void }) {
    ...
    const inputFileDir = utils.getFileDir(inputPath);
    ...
    for (;;) {
      ...
      if (await utils.checkDirWriteAccess(inputFileDir) && !simulateMasPermissionError) break;
