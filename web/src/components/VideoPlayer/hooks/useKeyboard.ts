...
export default ({ keyBindings, keyUpActions, getKeyboardAction, closeExportConfirm, exportConfirmOpen }: {
  ...
  const onKeyDownRef = useRef<((a: KeyEventParams) => void) | undefined>(undefined);
  useEffect(() => {
    onKeyDownRef.current = onKeyDown2;
  }, [onKeyDown2]);

  const onKeyUpRef = useRef<((a: KeyEventParams) => void) | undefined>(undefined);
  useEffect(() => {
  ...
  const updateKeyboardLayout = useCallback(async () => {
    console.warn('Keyboard layout update not implemented yet');
