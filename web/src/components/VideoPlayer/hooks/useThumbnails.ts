...
//bimport { renderThumbnails as ffmpegRenderThumbnails } from '../ffmpeg';
import type { Thumbnail } from '../types';
import { isDurationValid } from '../segments';
import { useInjection } from '../../../di/useInjection';
import { TOKENS, type IFfmpeg } from 'lossless-cut-application';
import { renderThumbnails } from '../ffmpeg';
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        await renderThumbnails({ signal: abortController.signal, filePath: debounced.filePath, from: debounced.zoomWindowStartTime, duration: debounced.zoomedDuration, onThumbnail: addThumbnail });
      } catch (err) {
        if ((err as Error).name !== 'AbortError' && !(/*isExecaError(err) &&*/ (err as any).isCanceled)) {
