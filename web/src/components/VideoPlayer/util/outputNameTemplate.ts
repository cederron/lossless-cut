...
import type { FileStats, FormatTimecode } from '../types';
import safeishEval from '../worker/eval';
import { TOKENS, type FileFfprobeMeta, type FileNameTemplateContext, type IPlatform, type IUtils, type SegmentToExport } from 'lossless-cut-application';
import { UserFacingError } from '../errors';
import { filenamify, hasDuplicates } from '../util';
  ...
  for (const fileName of fileNames) {
    ...
    if (safeOutputFileName) {
      if (platform.isWindows()) {
        ...
        invalidChars.add(utils.pathSep());
      }
    }

    const inPathNormalized = utils.pathNormalize(filePath);
    const outPathNormalized = utils.pathNormalize(utils.pathJoin(outputDir, fileName));
    const sameAsInputPath = outPathNormalized === inPathNormalized;
    const windowsMaxPathLength = 259;
    const shouldCheckPathLength = platform.isWindows() || isDev;
    const shouldCheckFileEnd = platform.isWindows() || isDev;

    if (utils.basename(filePath) === fileName) {
...
function maybeTruncatePath(fileName: string, truncate: boolean) {
  ...
  const pathSegs = fileName.split(utils.pathSep());
  ...
  return generateWithFallback({
    generate: async ({ template, safeOutputFileName: safeOutputFileName2, sanitizeName }) => {
      ...
      return pMap(segmentsToExport, async (segment, i) => {
        ...
        const inputFileNameWithoutExt  = utils.pathParsedName(sourceFile.path);
        ...
        const segFileName = await interpolateOutFileName(template, {
          ...
          sourceFiles: [sourceFile],
          inputFileNameWithoutExt,
          ext: utils.getOutFileExtension({ isCustomFormatSelected, outFormat: fileFormat, filePath: sourceFile.path }),
...
export async function generateMergedFileNames({ template: desiredTemplate, isCustomFormatSelected, fileFormat, sourceFiles, outputDir, safeOutputFileName, maxLabelLength, exportCount, epochMs }: {
  ...
  return generateWithFallback({
    generate: async ({ template, safeOutputFileName: safeOutputFileName2, sanitizeName }) => {
      const inputFileNameWithoutExt = utils.pathParsedName(firstFile.path);

      const fileName = await interpolateOutFileName(template, {
        epochMs,
        sourceFiles,
        inputFileNameWithoutExt,
        ext: utils.getOutFileExtension({ isCustomFormatSelected, outFormat: fileFormat, filePath: firstFile.path }),
        exportCount,
        segLabels: sourceFiles.map((file) => sanitizeName(utils.basename(file.path))),
      });
