...
import type { FindKeyframeMode, ParseTimecode } from '../types';
import { container } from 'tsyringe';
import type { IUtils } from 'lossless-cut-application/IUtils';
import { type IFfmpeg } from 'lossless-cut-application';

// const remote = window.require('@electron/remote');
// const { dialog } = remote;

const utils = container.resolve<IUtils>('Utils');
const ffmpeg = container.resolve<IFfmpeg>('Ffmpeg');
...
export const showOpenDialog = async () => {
  throw new Error('showOpenDialog is not supported in web version');
    // dialog.showOpenDialog({ ...props, title, ...(filters != null ? { filters } : {}) })
};
...
export async function askForInputDir(_defaultPath?: string | undefined) {
    throw new Error('askForInputDir is not supported in web version');
...
export async function askForOutDir(_defaultPath?: string | undefined) {
    throw new Error('askForOutDir is not supported in web version');
    //   const { filePaths } = await showOpenDialog({
//     properties: ['openDirectory', 'createDirectory'],
//     defaultPath: defaultPath!,
...
export async function askForFfPath(_defaultPath?: string | undefined) {
    throw new Error('askForFfPath is not supported in web version');
    //   const { filePaths } = await showOpenDialog({
...
export async function promptDownloadMediaUrl(outPath: string) {
  ...
  await ffmpeg.downloadMediaUrl(value, outPath);
  return true;
}
