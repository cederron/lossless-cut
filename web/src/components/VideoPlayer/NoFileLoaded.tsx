...
import type { KeyBinding, StateSegment } from 'lossless-cut-application';
    ...
    >
      ...
      {mifiLink && typeof mifiLink === 'object' && 'loadUrl' in mifiLink && typeof mifiLink.loadUrl === 'string' && mifiLink.loadUrl ? (
        <div style={{ position: 'relative', margin: '.3em', width: '24em', height: '8em' }}>
          ...
          <div style={{ width: '100%', height: '100%', position: 'absolute', cursor: 'pointer' }} role="button" onClick={(e) => { e.stopPropagation(); if ('targetUrl' in mifiLink && typeof mifiLink.targetUrl === 'string') /*electron.shell.openExternal(mifiLink.targetUrl);*/ throw new Error('External link opening is disabled'); }} />
        </div>
      ) : undefined}
    </motion.div>
  );
