...
import { llcProjectV1Schema, llcProjectV2Schema, type EdlExportType, type EdlFileType, type EdlImportType, type GetFrameCount, type LlcProject } from './types';
// import { llcProjectV1Schema, llcProjectV2Schema } from './types';
import { mapSaveableSegments } from './segments';
import isDev from './isDev';
import { TOKENS, type IUtils, type SegmentBase, type StateSegment } from 'lossless-cut-application';
...
async function loadCsv(path: string, parseTimeFn: (a: string) => number | undefined) {
  return parseCsv(trimBom(await utils.readFile(path, 'utf8')), parseTimeFn);
}

async function loadCutlistSeconds(path: string) {
  return parseCutlist(trimBom(await utils.readFile(path, 'utf8')));
}

async function loadXmeml(path: string) {
  return parseXmeml(await utils.readFile(path, 'utf8'));
}

async function loadFcpXml(path: string) {
  return parseFcpXml(await utils.readFile(path, 'utf8'));
}

async function loadDvAnalyzerSummaryTxt(path: string) {
  return parseDvAnalyzerSummaryTxt(trimBom(await utils.readFile(path, 'utf8')));
}

async function loadPbf(path: string) {
  return parsePbf(await utils.readFile(path));
}
